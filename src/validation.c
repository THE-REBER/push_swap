...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/10 18:31:02 by hmanilow          #+#    #+#             */
/*   Updated: 2022/02/10 18:31:02 by hmanilow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_it_num(char *str)
{
	...
	{
		if (!ft_isdigit(str[i]))
			ft_print_error("Incorrect input");
		i++;
	}
	return (0);
}
...
void	valid(int argc, char **argv, t_grenade *s_grenader)
{
	...
	while (i < argc)
	{
		...
		if (check_dublicate(&s_grenader->s_a, n))
			ft_print_error("Dublicate!");
		new_list = ft_lstnew(n);
		if (!new_list)
			ft_print_error("Memory wasn't allocated!");
		ft_lstadd_back(&s_grenader->s_a, new_list);
		s_grenader->length_a++;
		i++;
	}
	if (check_sort(&s_grenader->s_a))
		ft_print_error("isss sorted!");
}
